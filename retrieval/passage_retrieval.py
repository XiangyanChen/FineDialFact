...
import retrieval.src.index
import retrieval.src.contriever
import retrieval.src.utils
import retrieval.src.slurm
import retrieval.src.data
# from src.evaluation import calculate_matches
import retrieval.src.normalize_text
...
class Retriever:
        ...
        with torch.no_grad():
            for k, q in enumerate(queries):
                ...
                if args.normalize_text:
                    q = retrieval.src.normalize_text.normalize(q)
    ...
    def setup_retriever(self):
        ...
        self.model, self.tokenizer, _ = retrieval.src.contriever.load_retriever(self.args.model_name_or_path)
        self.model.eval()
        self.model = self.model.cuda()
        if not self.args.no_fp16:
            self.model = self.model.half()

        self.index = retrieval.src.index.Indexer(self.args.projection_size, self.args.n_subquantizers, self.args.n_bits)
        ...
        self.passages = retrieval.src.data.load_passages(self.args.passages)
    ...
    def setup_retriever_demo(self, model_name_or_path, passages, passages_embeddings, n_docs=5, save_or_load_index=False):
        ...
        self.model, self.tokenizer, _ = retrieval.src.contriever.load_retriever(model_name_or_path)
        self.model.eval()
        self.model = self.model.cuda()

        self.index = retrieval.src.index.Indexer(768, 0, 8)
        ...
        # load passages
        print("loading passages")
        self.passages = retrieval.src.data.load_passages(passages)
        self.passage_id_map = {x["id"]: x for x in self.passages}
        print("passages have been loaded")
...
if __name__ == "__main__":
    ...
    retrieval.src.slurm.init_distributed_mode(args)
    main(args)